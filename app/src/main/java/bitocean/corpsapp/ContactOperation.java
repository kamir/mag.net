...
public class ContactOperation {
    ...
    void addPhoto(ArrayList<ContentProviderOperation> ops){
        ...
        cv.put(ContactsContract.CommonDataKinds.Photo.PHOTO,ImageContact(R.drawable.ic_launcher));
    ...
    /*
    private Builder _createInsertForContact(long rawContactId, ContentValues cv) {
        Builder insertOp = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI).withValues(cv);
        if (rawContactId == -1) {
            insertOp.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0);
        } else {
            insertOp.withValue(ContactsContract.Data.RAW_CONTACT_ID, rawContactId);
        }
        return insertOp;
    }
    */
